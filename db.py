...
# ---------------- Mahsulot / To'lov ----------------
def mahsulot_qosh(mijoz_id, nom, narx, dona=1, sana=None):
    con = _con()
    cur = con.execute(
        "INSERT INTO mahsulotlar(mijoz_id,nom,narx,dona,sana,created) VALUES(?,?,?,?,?,?)",
        (mijoz_id, nom, float(narx or 0), float(dona or 1),
